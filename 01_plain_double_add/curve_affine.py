...
#        return pow(x, self.n - 2, self.n)
    ...
    def add(self, P, Q):
        ...
        if P == O_POINT_INF:
            ...
            if(self.generateLeakage):
                self.addSignal(P.x, self.addAmplitude)
                self.addSignal(P.y, self.addAmplitude)
                self.addSignal(Q.x, self.addAmplitude)
                self.addSignal(Q.y, self.addAmplitude)
            if P == Q:
    ...
    def double(self,P):
        ...
        if P == O_POINT_INF:
            ...
            if(self.generateLeakage):
                self.addSignal(P.x, self.doubleAmplitude)
                self.addSignal(P.y, self.doubleAmplitude)
            dydx = (3 * P.x**2 + self.a) * self.inv_mod(2 * P.y, self.p)
