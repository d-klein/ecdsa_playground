...
class ECC:
    ...
    def scalarMultMG(self, k, P):
        """
        scalar multiplication k*P on current curve; naive/non-secure using montgomery multiplication
        ...
        for i in range(start_index, len(k_bin)):
            #print("i" + str(i))
            if(k_bin[i] == '0'):
                #print("is 0")
                R1 = self.add(R0,R1)
